...
class Strategy(metaclass=ABCMeta):
    ...
    def _check_params(self, params):
        """子类需要设置属性的情形, 可以在策略初始化的时候传递过来, 这个函数会负责将参数赋值给对象实例实例"""
    ...
    def I(
        ...
        **kwargs,
        ...
        """
        # 首先检查指标名字, 如果指明了指标名字使用之, 没有的话就用函数名当做指标名字
        if name is None:
        ...
        # 如果现在指标值是一个 DataFrame, 此时意味着指标本身需要有一个值以上来表示
        # 此时转置指标数据, 使每一种值类型都成为一个 ndarray 用于后续处理
        ...
        # 通常情况下数据量是远多于指标的值数量的, 如果出现了 value.shape[0] > value.shape[1] 的情况,
        # 说明用户的指标<单个值列表>计算结果是以列形式提供过来的, 此时转置一下,
        # 都搞成以一行表示一个指标<单个值列表>的形式
...
class _Orders(tuple):
    ...
    def cancel(self):
        ...
        SL stands for Stop Loss, and TP stands for Take Profit.
        non-contingent orders: 非条件订单
        """
        for order in self:
...
class Order:
    ...
    def _replace(self, **kwargs):
        """知识点 Python name mangling: https://www.linkedin.com/pulse/role-underscore-python-mojtaba-ahmadpour"""
...
class _Broker:
    ...
    def new_order(
        ...
        *,
        ...
        """
        size = float(size)  # size 大小可以是一个百分比, 或者是数量值, 正数表示买入, 负数表示卖出
    ...
    def _adjusted_price(self, size=None, price=None) -> float:
        """
        这里将佣金体现在价格(返回值)里面, 操作完了之后会让卖价更便宜, 买价更贵
    ...
    def _process_orders(self):
        ...
        for order in list(self.orders):  # type: Order
            ...
            # 如果是多单(buy stop order)止损, 此时价格越高亏损越多, 因此价格高于 stop_price 的时候触发止损
            # 如果是空单(sell stop order)止损, 此时价格越低亏损越多, 因此价格低于 stop_price 的时候触发止损
            ...
            # 确定执行价格 + 检查限价单能否被填充
            if order.limit:
                # 先确认是否触及了限价价格, 如果没有触及, 那就不满足填充条件, 跳过订单的执行
                # 如果是 buy limit order, 那么只要价格低于指定价格就可以买入
                # 如果是 sell limit order, 那么只要价格高于指定价格即可卖出
                ...
                # 当一个 bar 里面同时能满足于 <stop, limit> 条件的时候, 会认为限价单先触发, 然后才是止损单
                # 这里的策略需要按照下面理解(用买单举例子):
                # 1. 下面 if 语句里面的情况, 限价 5 元, 止损设为 10 元, 大前提是限价先执行,
                #    我们用 5 元买入, 成本很低, 将来收益很高, 结果可能会太乐观, 因此这种情况不可取.
                # 2. 下面 if 语句里面不成立的情况, 限价 10 元, 止损设为 5 元, 此时以 10 元买入,
                #    结果比较悲观, 毕竟要在策略里面考虑最坏的情况.
                # 3. 若没有限价单先执行的假设, 上面的条件是不能成立的, 我们无法得到悲观的结果.
                #    但是在回测中要尽可能的往坏的方面考虑, 在实盘中才能取的更好的收益
                is_limit_hit_before_stop = is_limit_hit and (
                    order.limit < (stop_price or -np.inf)
                    if order.is_long
                    else order.limit > (stop_price or np.inf)
                )
                if is_limit_hit_before_stop:
                    continue
            ...
            if order.parent_trade:
                ...
                # 这里如果出现了 `订单数量` 大于 `交易数量` 的情况, 说明订单指定的数量在之前可能已经被填充过了
                size = copysign(min(abs(_prev_size), abs(order.size)), order.size)

                # If this trade isn't already closed (e.g. on multiple `trade.close(.5)` calls)
                # 如果 trade 已经被 close, 它就会被移除出 self.trades 列表
                if trade in self.trades:
                    self._reduce_trade(trade, price, size, time_index)
                    assert order.size != -_prev_size or trade not in self.trades

                # 断言确保 stop_limit 或者 take_profile 完美执行
                if order in (trade._sl_order, trade._tp_order):
                    ...
                    # order 已经被转换为一个 trade 了, 这里不需要再跟踪它
            ...
            # 如果交易数量是以百分比的形式指定的, 那么需要转换为真正的 `股数`
            size = order.size
            if -1 < size < 1:
                size = copysign(
            ...
            if not self._hedging:
                ...
                for trade in list(self.trades):
                    ...
                    if abs(need_size) >= abs(trade.size):
                        ...
                        need_size += trade.size  # 配对的反向交易 size 数量符号也是反的
                    else:
