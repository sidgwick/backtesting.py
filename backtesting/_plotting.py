...
def _maybe_resample_data(resample_rule, df, indicators, equity_data, trades):
    # 针对 df 数据重新采样, 主要是为了使数据调整到 _MAX_CANDLES 允许的数量范围之内
    if isinstance(resample_rule, str):
        freq = resample_rule
    else:
        # 没有指定采样规则, 或者数据本身长度就不够长, 就不需要重新采样了
    ...
    # 对交易进行重新采样:
    # TODO: 这应该是把交易追加到重新采样之后的那个 bar 上了吧? - _group_trades 函数做的应该就是这个事情
...
def plot(
    *,
    ...
    new_bokeh_figure: _figure = partial(
        _figure,
        x_axis_type="linear",
        width=plot_width,
        height=400,
    ...
    _kwargs = (
        dict(
            x_range=Range1d(
                index[0], index[-1], min_interval=10, bounds=(index[0] - pad, index[-1] + pad)
            )
        )
        if index.size > 1
        else {}
    )
    fig_ohlc: _figure = new_bokeh_figure(**_kwargs)
...
return this.labels[index] || "";
    ...
    def _plot_pl_section():
        ...
        set_tooltips(
            fig, tooltips + [("P/L", "@returns_long{+0.[000]%}")], vline=False, renderers=[r1]
        )
        set_tooltips(
            fig, tooltips + [("P/L", "@returns_short{+0.[000]%}")], vline=False, renderers=[r2]
        )
    ...
    fig_ohlc.x_range.js_on_change(
        "end", CustomJS(args=custom_js_args, code=_AUTOSCALE_JS_CALLBACK)  # type: ignore
    )
